  ...
  return (
    ...
    >
      ...
      {item.evidenceMetrics &&
        (item.evidenceMetrics.avgViews || item.evidenceMetrics.avgComments) && (
          <div className="mt-3 flex gap-4 text-xs text-[#64748b]">
            {item.evidenceMetrics.avgViews && (
              <span>평균 조회 {item.evidenceMetrics.avgViews.toLocaleString()}</span>
            )}
            {item.evidenceMetrics.avgComments && (
              <span>평균 댓글 {item.evidenceMetrics.avgComments}</span>
            )}
          </div>
        )}
...
export default function TimelineCards({ items }: Props) {

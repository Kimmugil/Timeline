...
function CustomTooltip({
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-3 text-sm max-w-xs shadow-lg">
      <p className="font-semibold text-slate-800 mb-2">{label}</p>

      {payload.map((p) => (
        <div key={p.name} className="flex justify-between gap-4 text-slate-500">
          <span style={{ color: p.color }}>{p.name}</span>
          <span className="text-slate-800 font-mono">{fmt(p.value)}</span>
        </div>
      ))}
      ...
      {events.length > 0 && (
        <div className="mt-3 border-t border-slate-100 pt-2 space-y-1">
          {events.slice(0, 3).map((ev) => (
            <div
              key={ev.id}
              className="flex items-start gap-1.5 cursor-pointer hover:text-slate-900 transition-colors"
          ...
          {events.length > 3 && (
            <p className="text-xs text-slate-400">외 {events.length - 3}건 →</p>
          )}
        </div>
      )}
    </div>
  ...
  return (
    <ResponsiveContainer width="100%" height={320}>
      ...
      >
        ...
        {hasMetrics && metricColumns.length > 0 && (
        ...
        {eventDates.map((date) => {
          ...
          return (
            <ReferenceLine
              ...
              strokeOpacity={selectedDate === date ? 0.9 : 0.35}
